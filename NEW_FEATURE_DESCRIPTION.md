...
本项目基于华为开源鸿蒙（HarmonyOS）多设备音乐界面示例工程 MusicHome 进行二次开发。原工程实现了基于自适应和响应式布局的多端音乐专辑应用，支持直板机、双折叠、平板和智能穿戴设备。在原工程基础上，我们新增了三大核心功能模块：**用户登录注册系统**、**设备间音乐文件传输**、以及**本地音乐文件管理**。
...
| 文件路径 | 说明 |
|---------|------|
| eatures/login/Index.ets | 模块导出入口 |
| eatures/login/src/main/ets/view/LoginPage.ets | 登录页面主组件 |
| eatures/login/src/main/ets/view/LoginHeader.ets | 登录页头部组件（含返回按钮和标题） |
| eatures/login/src/main/ets/view/LoginForm.ets | 登录表单组件（用户名、密码输入框及登录按钮） |
| eatures/login/src/main/ets/view/LoginInputItem.ets | 通用输入框组件（支持密码可见性切换） |
| eatures/login/src/main/ets/viewmodel/LoginViewModel.ets | 登录业务逻辑类 |
| eatures/login/src/main/ets/viewmodel/LoginModels.ets | 登录数据模型定义 |
| eatures/login/src/main/ets/constants/LoginConstants.ets | 登录常量、错误消息和响应码定义 |
| eatures/login/src/main/resources/base/element/string.json | 字符串资源 |
| eatures/login/src/main/resources/base/media/ic_eye_open.svg | 密码可见图标 |
| eatures/login/src/main/resources/base/media/ic_eye_close.svg | 密码隐藏图标 |
...
| 文件路径 | 说明 |
|---------|------|
| eatures/register/Index.ets | 模块导出入口 |
| eatures/register/src/main/ets/view/RegisterPage.ets | 注册页面主组件 |
| eatures/register/src/main/ets/view/RegisterForm.ets | 注册表单组件（用户名、密码、确认密码） |
| eatures/register/src/main/ets/viewmodel/RegisterViewModel.ets | 注册业务逻辑类 |
| eatures/register/src/main/ets/viewmodel/RegisterModels.ets | 注册数据模型定义 |
| eatures/register/src/main/ets/constants/RegisterConstants.ets | 注册常量、错误消息和响应码定义 |
| eatures/register/src/main/resources/base/element/string.json | 字符串资源 |
...
| 文件路径 | 说明 |
|---------|------|
| eatures/airdrop/src/main/ets/view/DeviceTransferPage.ets | 设备传输主页面（三步骤向导：选设备-选文件-传输） |
| eatures/airdrop/src/main/ets/view/DeviceListView.ets | 设备列表视图组件（设备发现、选择、刷新） |
| eatures/airdrop/src/main/ets/view/FilePickerPanel.ets | 文件选择面板组件（本地音频扫描、多选、格式筛选） |
| eatures/airdrop/src/main/ets/view/TransferProgressView.ets | 传输进度视图组件（进度条、速度、状态显示） |
| eatures/airdrop/src/main/ets/view/ReceiveConfirmDialog.ets | 接收确认对话框组件（超时自动拒绝） |
| eatures/airdrop/src/main/ets/view/TransferHistoryPage.ets | 传输历史页面组件（历史列表、详情查看、清空） |
| eatures/airdrop/src/main/ets/viewmodel/DeviceTransferViewModel.ets | 设备传输视图模型（协调设备发现、文件选择、传输执行） |
| eatures/airdrop/src/main/ets/viewmodel/FileSelectionService.ets | 文件选择服务（本地音频扫描、验证、筛选） |
| eatures/airdrop/src/main/ets/viewmodel/TransferHistoryService.ets | 传输历史服务（持久化存储、查询、统计） |
| eatures/airdrop/src/main/ets/viewmodel/NotificationService.ets | 通知服务（传输进度和结果通知） |
| eatures/airdrop/src/main/ets/viewmodel/TransferModels.ets | 传输数据模型定义 |
...
// 待传输文件
interface PendingTransferFile {
  fileId: string;
  fileName: string;
  filePath: string;
  fileSize: number;
  fileFormat: 'mp3' | 'wav' | 'flac' | 'aac' | 'm4a' | 'ogg' | 'other';
  duration?: number;
  isSelected: boolean;
}
...
- **观察者模式**：DeviceTransferViewModel 通过事件回调（DeviceTransferViewModelEvents）通知 UI 更新
